...
#define HIGHLIGHTED_BACKGROUND_COLOR [UIColor colorWithRed:0.40 green:0.80 blue:1.00 alpha:0.1]
#define DEFAULT_AUTOMATIC_RESIZE NO
#define DEFAULT_SHOW_TAG_MENU YES

@interface DWTagList () <DWTagViewDelegate> {
    void(^clickTagBlock)(NSString *, NSInteger);
}
...
- (void)touchUpInside:(id)sender
{
    ...
    if (self.showTagMenu && !clickTagBlock) {
        UIMenuController *menuController = [UIMenuController sharedMenuController];
        [menuController setTargetRect:tagView.frame inView:self];
        [menuController setMenuVisible:YES animated:YES];
        [tagView becomeFirstResponder];
    } else {
        clickTagBlock(tagView.label.text, tagView.tag);
    }
}

- (void) clickTagBlock:(void (^)(NSString *, NSInteger))block
{
    clickTagBlock = block;
}
